<template>
  <el-dialog :title="title" :width="width" top="50px" :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="isVisible">
    <el-form ref="form" :model="client" :rules="rules" label-position="right" label-width="165px">
      ...
      <el-form-item label="authorizedGrantTypes" prop="authorizedGrantTypes">
        <el-select v-model="client.authorizedGrantTypes" multiple value="" placeholder="" style="width:100%">
          <el-option v-for="item in grantTypes" :key="item.type" :label="item.type" :value="item.type" />
        </el-select>
      </el-form-item>
...
<script>
import { isIntegerGreaterThanZero, validURL } from '@/utils/validate'
